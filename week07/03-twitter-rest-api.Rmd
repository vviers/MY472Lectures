...
```{r}
getTimeline(filename="~/emmanuelmacron.json", screen_name="emmanuelmacron", n=3200, oauth=my_oauth)
```

What are the most common hashtags?
```{r}
tweets <- parseTweets("~/emmanuelmacron.json")
ht <- str_extract_all(tweets$text, '@[A-Za-z0-9_]+')
ht <- unlist(ht)
head(sort(table(ht), decreasing = TRUE), 30)
```
